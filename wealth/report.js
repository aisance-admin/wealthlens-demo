/* WealthLens · расчёт отчёта из прочитанного. Числа — только из выписок и курсов; здесь их складывают, пересчитывают в
   валюту отчёта и сверяют с итогами банка. Выводы Claude (сводка, пояснения) приходят отдельно и сюда не смешиваются. */
(function(){
...
/* Одна и та же бумага в двух таблицах одного файла (полный список и, например, «облигации ниже инвестиционного уровня»,
   которую Claude при чтении по частям принял за полный список): то же количество и та же стоимость на разных страницах плюс
...
/* Автоматические предупреждения: только то, что следует из чисел. Остальное добавляет Claude в сводке. */
function alerts(M, S){
...
/* Данные отчёта для Claude: сводка, выписки со сверкой, позиции (до 700 крупнейших), автоматические предупреждения. */
WL.compact = (M, S) => {
